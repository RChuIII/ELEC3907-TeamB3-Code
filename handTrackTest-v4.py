...
# Units dont matter
def getAngles(sideA: int, sideB: int, sideC: int) -> tuple:
    servoShoulderAngle = math.acos( (sideB**2 + sideC**2 - sideA**2) / (2*sideB*sideC) )
    servoElbowAngle = math.acos( (sideA**2 + sideC**2 - sideB**2) / (2*sideA*sideC) )
    return (servoShoulderAngle, servoElbowAngle)
    ...
    while cam_capture.isOpened():           # If the camera is still ON, keep the program running.
        ...
        if success:     # If a frame was captured successfully, continue, otherwise pass
            ...
            if results.multi_hand_landmarks:    # If there were hand landmarks found...
                ...
                cLength = calcDistance([dispWidth//2, dispHeight], [normX, normY])//20
                if cLength < 0:
                    print(getAngles(10,10,cLength))
                else:
                    print(getAngles(10,10,20))


#                claw = calcDistance(getCoords(4, (dispHeight, dispWidth)), getCoords(8, (dispHeight, dispWidth)))
#                if claw < 50:
#                    print("Claw actuated")
#                else:
#                    print("Claw not actuated")
