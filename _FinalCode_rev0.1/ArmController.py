...
def check_params(mode_select, claw_act, claw_rot):
    ...
    if (mode_select != 0.0):
        pos[2] = clamp(-data[0] * SCALING_FACTOR + 135, 0, 400)
        out[3] = clamp(servo_angle2PWM((data[1] + 8) * 6.875), 150, 420)
    else:
        pos[0] = clamp(data[0] * SCALING_FACTOR, -300, 300)
        pos[1] = clamp(-data[1] * SCALING_FACTOR, -300, 300)
    ...
    pos[0] = clamp(pos[0],0.1,10000)
    out[0:2] = roboArm.get_joint_angles(pos)

    angle_cur = out[0:2]
    out[0] = out[0] / 1.8                         # Divide base ANLGE by 1.8 to get the number of steps
