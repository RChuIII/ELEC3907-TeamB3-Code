"""
    ...
    Lasted Edited on : Feburary 06, 2023
...
import serial # Send data via serial
import struct # struct.pack data into bytes

mp_hands = mp.solutions.hands                       # Used for all of the hand tracking data
serialCon = serial.Serial(port='COM3', baudrate=9600, timeout=None) # Serial Communication with Arduino
    ...
    while cam_capture.isOpened():           # If the camera is still ON, keep the program running.
        ...
        if success:     # If a frame was captured successfully, continue, otherwise pass
            ...
            if results.multi_hand_landmarks:    # If there were hand landmarks found...
                ...
                serialCon.write(struct.pack("f", baseAngle))        # Send stepper Angle
                serialCon.write(struct.pack("f", jointAngles[0]))   # Send shoulder servo angle
                serialCon.write(struct.pack("f", jointAngles[1]))   # Send elbow servo angle
                serialCon.write(struct.pack("f",0000.0))            # Send data terminator
